	...
	for mapId, data in pairs( Nx.GuideData[typ][cont] or {}) do
		...
		for i, fxy in pairs(dataTbl) do
			...
			if fac ~= hideFac then
				...
				local name, locName = Nx.compat:strsplit (",", namecamp)
				local wx, wy = Map:GetWorldPos (mapId, x, y)
				local node = {}
				locName = Nx.compat:strsub(locName,2)
				node.Name = namecamp
...
function Nx.Travel:CaptureTaxi()
	...
	for n = 1, NumTaxiNodes() do
		...
		local locName = Nx.compat:strsplit (",", TaxiNodeName (n))
...
function Nx.Travel.TakeTaxiNode (node)
	...
	map.TaxiName = Nx.compat:strsplit (",", TaxiNodeName (node))
	...
	if num > 0 then
		...
		for n = 1, rCnt do
			...
			if srcNode and destNode then
				...
				local srcName = Nx.compat:strsplit (",", TaxiNodeName (srcNode))
				local destName = Nx.compat:strsplit (",", TaxiNodeName (destNode))
...
function Nx.Travel:TaxiFindConnectionTime (srcName, destName)
	...
	for i, stenti in pairs(Nx.FlightConnection) do
		...
		local stName = Nx.compat:strsplit (",", st)
		local enName = Nx.compat:strsplit (",", en)
...
--		Nx.prtCtrl ("** path nodes start %s to %s", srcMapId, dstMapId)
		...
		repeat
			...
			local nodeCnt = Nx.compat:tsize (path)

			for n = 1, Nx.compat:tsize (path) - 1 do
...
--				Nx.prt ("path watchdog")
		...
		until nodeCnt == Nx.compat:tsize (path)

		-- Build path

--		Nx.prtCtrl ("path nodes %s", #path)

		for n = 2, Nx.compat:tsize (path) - 1 do
...
--		local name = format ("Fly: %s to %s", t1Node.Name, bt2Node.Name)
		...
		node1.Tex = "Interface\\Icons\\Ability_Druid_FlightForm"
		tinsert (path, node1)
		...
		node2.Y = bt2Node.WY
		node2.Name = name
		node2.Tex = "Interface\\Icons\\Ability_Druid_FlightForm"
		tinsert (path, node2)
...
--				Nx.prt ("Close %s %d (%s %d %d)", node.Name, dist ^ .5, mapId, posX, posY)
	...
	if closeNode then
		local tex = "Interface\\Icons\\Ability_Druid_FlightForm"
...
--		Nx.prtCtrl ("C %s to %s #%s", Nx.MapIdToName[srcMapId], Nx.MapIdToName[dstMapId], #zcon)
		...
		if Nx.compat:tsize (zcon) == 0 then
