...
--		Nx.prt ("SocialEnable")
		...
		-- Nx.compat:hooksecurefunc ("PanelTemplates_SetTab", Nx.Social.PanelTemplates_SetTab)
	end
...
--		Nx.prtFrame ("FF", frame)
		...
		if Nx.compat:InCombatLockdown() and(Nx.compat:GetNumGroupMembers()>0 or _G["RaidGroupFrame_Update"]) then
			return
		end
...
function Nx.Social.List:Menu_OnPunkSetNote()
	...
	if self.MenuSelName then
		...
		if punk then
			...
			local tm, lvl, class, note = Nx.compat:strsplit ("~", punk)
...
--PAIDS!
		...
		local function func (a, b)
			local pName1, fName1 = Nx.compat:strsplit ("~", a)
			local pName2, fName2 = Nx.compat:strsplit ("~", b)
		...
		for _, plyr in ipairs (data) do
			...
			local pName, fName = Nx.compat:strsplit ("~", plyr)
			local i = fI[fName]
			...
			if Nx.compat:tsize (pName) > 0 then
			...
			if connected then
				...
				level, class = Nx.compat:strsplit ("~", pData[fName])
			end
...
--PAIDE!
	...
	elseif tabI == 2 then
		...
		for _, pName in ipairs (data) do
			...
			local tm, lvl, class, note = Nx.compat:strsplit ("~", punks[pName])
			...
			if note and Nx.compat:tsize (note) > 0 then
				list:ItemSet (6, note)
			end
		end
...
--		Nx.prtVar ("", data)
		...
		for _, pName in ipairs (data) do
			...
			list:ItemSet (1, format ("%d:%02d", mod(seconds / 60 , 60), mod(seconds , 60)))
	...
	elseif NxData.NXVerDebug and tabI == 3 then
		...
		for n, msg in ipairs (data) do
			...
			local name, ver, r, c, dt, ver1, qCnt, lvl, mId = Nx.compat:strsplit ("%^", msg)
			ver = tonumber (ver)
...
--	Nx.prt ("%s: %s %s %s", finderName, punksStr, info.X, info.Y)
	...
	local punksT = { Nx.compat:strsplit ("!", punksStr) }

	for n, v in ipairs (punksT) do

		local lvl = tonumber (Nx.compat:strsub (v, 1, 2), 16)
		if not lvl then	-- Error?
			break
		end

		local name = Nx.compat:strsub (v, 3)
		if lvl >= tonumber("0xff", 16) then
			name = Nx.compat:strsub (v, 9)
			lvl = 0
		end
...
--	Nx.prt ("AddLocalPunk %s", name)
	...
	name = Nx.compat:strmatch (name, "[^-]+")		-- Remove server name
	...
	if plyrNear then
		...
		plyrNear = Nx.compat:strmatch (plyrNear, "[^-]+")		-- Remove server name
...
--	punk.Alert = nil
	...
	if not punk.Alert and self.Punks[name] then
		...
		if self.GOpts["PunkMAlertText"] then
			...
			local tm, lvl, class, note = Nx.compat:strsplit ("~", self.Punks[name])
...
function Nx.Social:UpdateIcons (map)
	...
	if mod(Nx.Tick , 120) == 4 then
		self:CalcPunks()
	end

	self.GOpts = self.GOpts or Nx.GetGlobalOpts()
	local math = math
	...
	local punks = self.Punks
	local punksA = self.PunksActive or {}
	...
	local iconGlow = abs (mod(GetTime() * 400 , 200) - 100) / 400 + .75
...
--			if ((punkMId ~= 3006 and punkMId ~= 4003 and punkMId ~= 4012) or showInSafeArea) and
		...
		if punks[pName] then	-- Punk match?
			...
			if map:ClipFrameW (f, x, y, 14, 14, 0) then
				...
				f.NxTip = format ("*|cffff0000%s %s, %d:%02d ago\n%s (%d,%d)", pName, lvl, mod(dur / 60 , 60), mod(dur , 60), mapName, punk.X, punk.Y)
				f.NXType = 3001
				f.NXData = pName
...
--		Nx.prt ("But #%s create", n)
		...
		local but = CreateFrame ("Button", nil, win.Frm)
		self.Buts[n] = but
...
--PAIDS!
	...
	if not self.Punks[name] then
		...
		if punks[name] then		-- Match?
			...
			local found
			for n = 1, Nx.compat:tsize (self.Punks) do
...
--PAIDS!
	...
	if self.Changed then
		...
		local lockDown = Nx.compat:InCombatLockdown() ~= nil
...
--				Nx.prt ("But #%s update %s", n, name)
				...
				local function func (self)
					TargetByName(this.NXName)
				end
				but:SetScript("OnMouseDown" , func)
...
--PAIDS!
	...
	if update and not lockDown then
		...
		if Nx.compat:IsInRaid() then		-- Arenas are raids
