...
function Nx.Fav:RecordAnimTimer()
	...
	if self.Recording then
		local a = mod((self.RecAlphaAnim - 35) , 1000)
		self.RecAlphaAnim = a
...
function Nx.Fav:IMenu_OnPaste()
	...
	local fav = self.CurFav
	if fav then
		local i = min (self.CurItemI, Nx.compat:tsize (fav)) + 1
...
--	Nx.prt ("List event "..eventName)
	...
	elseif eventName == "button" then	-- Button icon
		...
		self:SetItemFlags (sel, tonumber("0xfe", 16), flags)
...
function Nx.Fav:AddFolder (name, parent, index)
	...
	if parent then
		index = index or Nx.compat:tsize (parent) + 1
...
--	Nx.prt ("Fav AddItem %s #%s %s", fav["Name"], index or "nil", item)
	...
	if fav then
		...
		local i = max (min (index or 999999, Nx.compat:tsize (fav)), 0) + 1
		tinsert (fav, i, item)
...
function Nx.Fav:MakeXY (x, y)
	local s = Nx:PackXY (x, mod(y , 100))
	return s .. strchar (floor (y / 100) + 35)	-- Add dungeon level to end
end

function Nx.Fav:ParseItem (item)
	if item then
		return Nx.compat:strsplit ("~", item)	-- Type~Flags (# based)~Name~Data
	end
end
...
function Nx.Fav:ParseItemNote (data)
	...
	local zone = tonumber (Nx.compat:strsub (data, 2, 3), 16)
	local id = Nx.NxzoneToMapId[zone]
	local x = tonumber (Nx.compat:strsub (data, 4, 6), 16) / 4090 * 100
	local y = tonumber (Nx.compat:strsub (data, 7, 9), 16) / 4090 * 100
...
function Nx.Fav:ParseItemTarget (data)
	...
	local zone = tonumber (Nx.compat:strsub (data, 1, 2), 16)
	local id = Nx.NxzoneToMapId[zone]
	local x = tonumber (Nx.compat:strsub (data, 3, 5), 16) / 4090 * 100
	local y = tonumber (Nx.compat:strsub (data, 6, 8), 16) / 4090 * 100
...
function Nx.Fav:GetItemTypeName (index)
	...
	local fav = self.CurFav
	if fav then
		local typ, flags, name = Nx.compat:strsplit ("~", fav[index])
		return typ, name
	end
end
...
function Nx.Fav:SetItemFlags (index, mask, orFlags)
	...
	local fav = self.CurFav
	if fav then
		local typ, flags, name, data = Nx.compat:strsplit ("~", fav[index])
...
function Nx.Fav:SetItemName (index, name)
	...
	local fav = self.CurFav
	if fav then
		local typ, flags, _, data = Nx.compat:strsplit ("~", fav[index])
		if data then
...
function Nx.Fav:SelectItems (index)
	...
	if self.CurFav then
		...
		self.CurItemI = min (index, Nx.compat:tsize (self.CurFav))
...
function Nx.Fav:SetNoteAtStr (str)
	...
	for s in Nx.compat:gmatch (str, ".") do
...
--		Nx.prt ("%s", s)
		...
		if strDone then
			if Nx.compat:tsize (curStr) > 0 then
	...
	if Nx.compat:tsize (words) > 1 then
...
function Nx.Fav:UpdateTargets()
	...
	if self.CurFav and self.CurItemI	and (self.Recording or shown) then
		...
		for n = self.CurItemI, Nx.compat:tsize (self.CurFav) do
...
function Nx.Fav:UpdateIcons()
	...
	if self.CurFav and self.CurItemI then
		...
		if typ == "N" then
			...
			map:SetIconTypeAlpha ("!Fav2", abs (( mod(GetTime() * 100 , 100) - 50) / 50))
		end
