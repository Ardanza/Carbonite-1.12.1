...
--	Nx.Timer:Start ("ComVerTest", 3, self, function() self:ShowVersionMsg() end)
	...
	Nx.compat:hooksecurefunc ("SendChatMessage", self.SendChatHook)
...
--	Nx.prt ("ComChatEvent: %s %s", event, arg9)
	...
	if Nx.compat:strsub (arg9, 1, 3) == self.Name then
		...
		elseif event == "CHAT_MSG_CHANNEL_NOTICE" then
			...
			local nameRoot = Nx.compat:strsplit ("I", arg9)	-- Drop I and #

			if arg1 == "YOU_JOINED" then

				local typ = strupper (Nx.compat:strsub (arg9, 4, 4))
...
--					Nx.prt ("Join %s", arg9)
				...
				elseif typ == "Z" then
					...
					local mapId = tonumber (Nx.compat:strsub (nameRoot, 5))
					if mapId then
...
--			self.List:Update()
			...
			if id == 83 then		-- S (status) Check 1st for performance
				...
				if not self.PalsInfo[name] then	-- Not a pal we have?
					if Nx.compat:tsize (msg) >= 16 then
...
--	Nx.prt ("ComChatAddonEvent: %s %s %s", event, arg1, arg4)
	...
	if Nx.compat:strsub (arg1, 1, 3) == self.Name then
...
--			self.List:AddInfo ("A:"..arg1, format ("(%s %s) %s", name, arg3, arg2))
			...
			local data = { Nx.compat:strsplit ("\t", arg2) }
...
function Nx.Com:ParsePlyrStatus (name, info, msg)
	...
	local mapId = tonumber (Nx.compat:strsub (msg, 3, 6), 16)	-- Map id
...
--		Nx.prt ("Die %s", mapId)
	...
	info.X = tonumber (Nx.compat:strsub (msg, 7, 9), 16) / tonumber("0xfff", 16) * 100
	info.Y = (tonumber (Nx.compat:strsub (msg, 10, 13), 16) or 0) / tonumber("0xfff", 16) * 100		-- Includes dungeon level offset (dzzz)
...
--		end
		...
		info.TName = Nx.compat:strsub (msg, 22, 22 + len - 1)
	...
	if bit.band (flags, 4) > 0 then
		...
		local len = Nx.Quest:DecodeComRcv (info, Nx.compat:strsub (msg, off))
	...
	if bit.band (flags, 8) > 0 then
		...
		Nx.Social:DecodeComRcvPunks (name, info, Nx.compat:strsub (msg, off + 1))
	end
end
...
function Nx.Com:ParseLGP (name, msg)
	...
	if strbyte (msg) == tonumber("0x50", 16) then	-- P (position)

		local x, x2, y, y2, len = Nx.compat:strbyte (msg, 2, 6)

		if len and len > 1 then

			x = ((x - 1) * 255 + x2 - 1) / (255 ^ 2) * 100
			y = ((y - 1) * 255 + y2 - 1) / (255 ^ 2) * 100
			local zoneName = Nx.compat:strsub (msg, 7, 5 + len)	-- Len is +1 real length
...
--			Nx.prt ("Leave Chan N %d %s", id, name)
			...
			local name3 = Nx.compat:strsub (name, 1, 3)
			if name3 == self.Name then

				local typ = strupper (Nx.compat:strsub (name, 4, 4))
				if typ == typeName then

					if typ == "Z" then

						local nameRoot = Nx.compat:strsplit ("I", name)	-- Drop I and #
						local id = tonumber (Nx.compat:strsub (nameRoot, 5))
...
--			Nx.prt ("Com scan %s", name)
			...
			local name4 = Nx.compat:strsub (name, 1, 4)
	...
	for n = 1, 10 do
		...
		if name then
			...
			if name3 == self.Name then
				...
				local typ = Nx.compat:strsub (name, 4, 4)
				if typ == typeName then
...
function Nx.Com:SendPals (msg)
	...
	self.PalsSendQ[Nx.compat:tsize (self.PalsSendQ) + 1] = msg
end
...
function Nx.Com:SendSecW (pre, msg, plName)
	...
	self.SentBytes = self.SentBytes + Nx.compat:tsize (str) + 54 + 20	-- Packet overhead + some WOW overhead
...
function Nx.Com:Send (chanId, msg, plName)
	...
	if chanId == "Z" then	-- Zone chat channel
		...
		self.SentBytes = self.SentBytes + Nx.compat:tsize (msg) + 54 + 20	-- Packet overhead + some WOW overhead

		if chanId == "g" then	-- Addon guild

			if IsInGuild() then
				-- Nx.prt("Guild addon msg")
				SendAddonMessage (self.Name, msg, "GUILD")
			end

		elseif chanId == "p" then	-- Addon party
			-- Nx.prt("Party addon msg")
...
function Nx.Com:Chksum (msg)
	...
	for n = 1, Nx.compat:tsize (msg) do
...
--	Nx.prt ("Com chksum rcv %s", #msg)
	...
	if Nx.compat:tsize (msg) >= 4 then
		...
		for n = 5, Nx.compat:tsize (msg) do
			 v = xor (v, strbyte (msg, n))
		end
...
function Nx.Com:Encode (msg)
	...
	s[1] = Nx.compat:strsub (msg, 1, 2)

	for n = 3, Nx.compat:tsize (msg) do
...
--			Nx.prt ("chat OR in %s (%s)", Nx.compat:strsub (msg, 1, 2), Nx.compat:strsub (msg, s1, s1 + 5))
			msg = gsub (msg, "|", "\1")
		end
	end
...
function Nx.Com:HostileExtractName( msg )
...
function Nx.Com:OnCombat_hostile_event (event, msg)

	local name = Nx.Com:HostileExtractName(msg)
	-- Nx. prt ("OnCombat_hostile_event %s: %s", event, msg)
	if name then
		Nx.Social:AddLocalPunk (name)
...
--	Nx.prt ("send delay %s", delay)
	...
	if tm - self.SendPosTime >= delay then
		...
		x = max (min (self.PlyrX,  .999), 0) * tonumber("0xfff", 16)
		y = max (min (self.PlyrY, 9.999), 0) * tonumber("0xfff", 16)
...
--			Nx.prt ("THealth %s", hper)
			...
			tStr = format ("%c%c%c%c%c%s", tType+35, tLvl+35, tCls+35, hper+35, Nx.compat:tsize (targetName)+35, targetName)
		end
...
--				enStr = enStr .. format ("%2x%s!", lvl, name)	-- Old, sends 0xffffffff for -1 level
				...
				if Nx.compat:tsize (enStr) > 50 then
					break
				end
			end
			...
			enStr = strchar (Nx.compat:tsize (enStr) - 1 + 35) .. Nx.compat:strsub (enStr, 1, -2)
...
--]]
	...
	if not self.PalsSendMsg then
		...
		if Nx.compat:tsize (self.PalsSendQ) > 0 then

			self.PalsSendMsg = self.PalsSendQ[1]
			self.PalsSendQ[1] = nil

			for n = 2, Nx.compat:tsize (self.PalsSendQ) do
...
--				table.remove (self.PalsSendQ, 1)
	...
	if tdiff >= .25 then
		...
		if msg then
			...
			if self.PosSendNext > Nx.compat:tsize (self.Friends) then
	...
	if Nx:FindActiveChatFrameEditBox() then
		...
		if tm - self.SendChanTime >= .5 then
			...
			if self.SendChanQ[1] then
				...
				self.SentBytes = self.SentBytes + Nx.compat:tsize (data.Msg) + 54 + 20	-- Packet overhead + some WOW overhead
...
--	Nx.prt ("Ver %s", msg)
	...
	if self:IsChksumOK (msg) then
		...
		local subType = Nx.compat:strsub (msg, 2, 2)

		if subType == " " then	-- Global version?

			local ver, r, c, dt, ver1, qCnt = Nx.compat:strsplit ("%^", msg)

			ver = tonumber (Nx.compat:strsub (ver, 5))
			if ver then
...
function Nx.Com:RcvVersion (name, msg)
	...
	if NxData.NXVerDebug then
		...
		local ver, r, c, dt, ver1, qCnt, lvl, mapId = Nx.compat:strsplit ("%^", msg)
		ver = tonumber (Nx.compat:strsub (ver, 5))
...
--	Nx.prt ("ShowVerTimer %s %s = %s", lasttm or 0, tm, Nx.compat:difftime (tm, lasttm or 0))

	if not lasttm or Nx.compat:difftime (tm, lasttm) > 4 * 3600 then	-- Hours since last message?
...
function Nx.Com:OnGetUserVerTimer()
	...
	if i <= Nx.compat:tsize (self.GetUserVerNames) then
...
function Nx.Com.List:Update()
	...
	self.Win:SetTitle (format ("Com %d Bytes sec %d", Nx.compat:tsize (self.Sorted), Nx.Com.SentBytesSec or 0))
