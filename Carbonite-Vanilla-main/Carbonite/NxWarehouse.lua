...
--	self.LSecondarySkills = gsub (SECONDARY_SKILLS, ":", "")
	...
	self.RidingSpells = {
		[75]  = "Apprentice Riding", --GetSpellInfo (33389) or "",
		[150] = "Journeyman Riding", -- GetSpellInfo (33392) or "",
...
function Nx.Warehouse:prtdb (arg1, arg2, arg3, arg4, arg5, arg6)
	if self.Debug then
		Nx.prt (arg1, arg2, arg3, arg4, arg5, arg6)
	end
end
...
function Nx.Warehouse:Menu_OnImport (item)
	...
	if cn > 1 and rc then
		...
		local rname, sname = Nx.compat:strsplit (".", rc)
...
--	local prof = floor (data / 1000)
	...
	if (id >= 1 and id <= Nx.compat:tsize (Nx.RealmChars)) or id == 99 then
	...
	elseif eventName == "button" then	-- Button icon
		...
		elseif id >= 1 and id <= Nx.compat:tsize (Nx.RealmChars) then
...
--					Nx.Item:ShowTooltip (id, true)
			...
			elseif id == 0 then
				...
				if tip then
					tip = Nx.compat:strsub (tip, 2)	-- Remove !
					...
					if str then
						...
						local id = Nx.compat:strmatch (tip, "item:(%d+)")
...
--		if IsShiftKeyDown() then
			...
			if tip then
				...
				link = Nx.compat:strsub (tip, 2)	-- Remove !
...
function Nx.Warehouse:Update()
	...
	self.Win:SetTitle (format ("Warehouse: %d characters", Nx.compat:tsize (Nx.RealmChars)))
...
--			ch["Class"] = "Deathknight"	-- TEST
			...
			if not hide then
				...
				if cname == myName then		-- Me?
					...
					local secs = Nx.compat:difftime (time(), ch["LTime"])
					local mins = mod(secs / 60 , 60)
					local hours = secs / 3600
					local lvlHours = Nx.compat:difftime (time(), ch["LvlTime"]) / 3600
					local played = Nx.Util_GetTimeElapsedStr (ch["TimePlayed"])

					list:ItemAdd (cnum)
					list:ItemSet (2, format (" Time On: %s%2d:%02d:%02d|r, Played: %s%s", hicol, hours, mins, mod(secs , 60), hicol, played))
					...
					if ch["Time"] then
						...
						local secs = Nx.compat:difftime (time(), ch["Time"])
					...
					if ch["Pos"] then
						...
						local mid, x, y = Nx.compat:strsplit ("%^", ch["Pos"])
				...
				if lvl < MAX_PLAYER_LEVEL then
					if ch["XP"] then
						...
						if ch["Time"] then
							rest = min (150, rest + Nx.compat:difftime (time(), ch["Time"]) * .0001736111)
						end
...
--[[
				if ch["Professions"] then
					for n, data in ipairs (ch["Professions"]) do
						local name, rank = Nx.compat:strsplit ("%^", data)
						list:ItemAdd (cnum)
...
function Nx.Warehouse:UpdateItems()
	...
	cn2 = Nx.compat:tsize (Nx.RealmChars)
	...
	if self.SelectedChar ~= 99 then
		...
		if inv then
			...
			for _, data in ipairs (inv) do
				...
				local slot, link = Nx.compat:strsplit ("%^", data)
				Nx.Item:Load (link)
...
--]]
	...
	for name, data in pairs (items) do
		...
		local bagCnt, bankCnt, mailCnt, link = Nx.compat:strsplit ("%^", data)
		Nx.Item:Load (link)

		if self.SortByRarity or self.SortBySlot then

			local _, iLink, iRarity, minLvl, itype, _, _, equipLoc = GetItemInfo (link)
	...
	if not self.ShowItemCategory then
		...
		for _, v in ipairs (isorted) do
			...
			local _, name, bagCnt, bankCnt, mailCnt, link = Nx.compat:strsplit ("%^", v)
...
--[[
			local name, iLink, iRarity, minLvl, itype = GetItemInfo (link)
			Nx.prt ("item %s", itype)
--]]
		end
	else
		...
		for _, typ in ipairs (self.ItemTypes) do
			...
			for n = 1, Nx.compat:tsize (isorted) do

				local _, name, bagCnt, bankCnt, mailCnt, link = Nx.compat:strsplit ("%^", isorted[n])
				local _, iLink, iRarity, minLvl, itype = GetItemInfo (link)
				...
				if itype == typ then	-- Found one of type?
					...
					for n2 = n, Nx.compat:tsize (isorted) do

						local _, name, bagCnt, bankCnt, mailCnt, link = Nx.compat:strsplit ("%^", isorted[n2])
						local _, iLink, iRarity, minLvl, itype = GetItemInfo (link)
...
function Nx.Warehouse:AddItem (items, typ, name, data)
	...
	if items[name] then
		totalBag, totalBank, totalMail = Nx.compat:strsplit ("%^", items[name])
	end

	local count, iLink = Nx.compat:strsplit ("%^", data)
...
function Nx.Warehouse:UpdateItem (pre, name, bagCnt, bankCnt, mailCnt, link, showILvl)
	...
	local iname, iLink, iRarity, minLvl, itype, subType, stackCount, equipLoc, tx = GetItemInfo (link)
...
--				Nx.prt ("Owners %s", self.ItemOwners)
				...
				str = Nx.compat:strsub (self.ItemOwners, pos, e and e - 1)
...
--	Nx.prt ("Find Link %s", link)
	...
	for cnum, rc in ipairs (Nx.RealmChars) do
		...
		if bags then
			for name, data in pairs (bags) do
				local iCount, iLink = Nx.compat:strsplit ("%^", data)
		...
		if inv then
			for name, data in pairs (inv) do
				local slot, iLink = Nx.compat:strsplit ("%^", data)
				local s1, s2, iLink = strfind (iLink, "item:(%d+)")
				if iLink == link then
					invCnt = invCnt + 1
				end
	...
	if profT then
		...
		for id, itemId in pairs (profT) do
			...
			if type (id) == "number" then
				...
				local iName, iLink, iRarity, iMinLvl, iType, iSubType, iStackCount, iEquipLoc = GetItemInfo (itemId)
...
--				if self.SortBySlot and iEquipLoc then
		...
		for _, str in ipairs (items) do
			...
			local cat, _, name, id = Nx.compat:strsplit ("%^", str)
			local id = tonumber (id)

			local link = GetSpellLink (id)
			local iName, iLink, iRarity, iMinLvl, iType, iSubType, iStackCount, iEquipLoc, iTx
			local col = ""
			...
			if profT[id] > 0 then
				...
				iName, iLink, iRarity, iMinLvl, iType, iSubType, iStackCount, iEquipLoc, iTx = GetItemInfo (itemId)
				if iRarity then
					iRarity = min (iRarity, 6)		-- Fix Blizz bug with color table only going to 6. Account bound are 6 or 7
...
function Nx.Warehouse:AddLink (link, count, inv)
	...
	if name and inv then		-- inv can somehow be nil. bank addon?
		...
		if inv[name] then
			total = Nx.compat:strsplit ("%^", inv[name])
		end
...
function Nx.Warehouse.OnLoot_slot_cleared()
	...
	if self.LootItems[arg1] then
		local name, iLink, iRarity, minLvl, iType = GetItemInfo (self.LootItems[arg1])
		if iType == "Quest" then
...
function Nx.Warehouse:DiffBags (oldBags)
	...
	for name, v in pairs (ch["WareBags"]) do
		...
		local newCnt, link = Nx.compat:strsplit ("%^", v)

		if oldBags[name] then
			local oldCnt = Nx.compat:strsplit ("%^", oldBags[name])
...
--	Nx.prt ("Trade %s", title)
	...
	for n = 1, cnt do
		...
		if typ ~= "header" then
			...
			local rId = link and Nx.compat:strmatch (link, "enchant:(%d+)")

			local link = GetTradeSkillItemLink (n)
			local itemId = link and Nx.compat:strmatch (link, "item:(%d+)") or 0
