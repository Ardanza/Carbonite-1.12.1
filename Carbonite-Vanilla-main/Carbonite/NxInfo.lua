...
--	Nx.prt ("Guide list event "..eventName)
	...
	if eventName == "update"  then
		...
		local t = { Nx.compat:strsplit ("&", data) }

		for _, str in ipairs (t) do

--			Nx.prt ("info %s", str)

			local v1, v2, v3 = Nx.compat:strsplit ("%^", str)
...
function Nx.Info:UpdateItems()
	...
	for index, val in ipairs (items) do
		...
		local data = Nx.compat:strsplit ("%^", val)
...
--			local s1, s2, cap, extra = strfind (data, "<(%a+%%?)([^>]*)>", pos)
			...
			if s1 then
				if s1 > pos then
					str = str .. Nx.compat:strsub (data, pos, s1 - 1)
				end

--				Nx.prtCtrl ("item cap %s", cap)

				if Nx.compat:tsize (cap) == 1 then
					...
					local cmd, v1, v2, v3, v4 = Nx.compat:strsplit (";", cap)	-- FIX for multiple params!
				...
				str = str .. Nx.compat:strsub (data, pos)	-- Grab end
				break
			end
		end
		...
		if Nx.compat:tsize (str) > 0 then

--			Nx.prtStrHex ("Info:", str)

			list:ItemAdd (index)

			local str, extra = Nx.compat:strsplit ("~", str)
...
--	Nx.prtVar ("BGQ", num)
	...
	if status == "queued" then
		...
		if i >= 0 then
			return "", format ("%s ETA %d:%02d", name, mod(i / 60 , 60), mod(i , 60))
		else
			return "|cffff0000", format ("%s ETA -%d:%02d", name, mod(-i / 60 , 60), mod(-i , 60))
		end
	...
	elseif status == "confirm" then
		...
		return "|cff00ff00", format ("%s cancel %d:%02d", name, mod(i / 60 , 60), mod(i , 60))
	end
end
...
function Nx.Info:CalcBGStart()
	...
	if Nx.InBG and info.BGStartTime then
		...
		if i >= 0 then
			return "|cff8080ff", format ("%d:%02d", mod(i / 60 , 60), mod(i , 60))
		else
...
function Nx.Info:CalcBGDuration()
	...
	if Nx.InBG then
		...
		if i > 0 then
			return "", format ("%d:%02d", mod(i / 60 , 60), mod(i , 60))
		end
	end
end
...
--	local i = GetWintergraspWaitTime()
	if not active and i and i > 0 then
		i = i / 60
		s = ""
		s2 = format ("%d:%02d", mod(i / 60 , 60), mod(i , 60))
	end
	...
	if not active and i and i > 0 then
		i = i / 60
		if s2 then
			s2 = s2 .. format (", TB %d:%02d", mod(i / 60 , 60), mod(i , 60))
		else
			s = ""
			s2 = format ("TB %d:%02d", mod(i / 60 , 60), mod(i , 60))
		end
	end
