...
function Nx.prt (msg, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10)
	local f = Nx.prtChatFrm or DEFAULT_CHAT_FRAME
	f:AddMessage (Nx.TXTBLUE..NXTITLE.." |cffffffff".. (format (msg, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10) or "nil"), 1, 1, 1)
end

function Nx.prtError (msg, arg1, arg2, arg3, arg4, arg5, arg6)
	UIErrorsFrame:AddMessage (format (msg, arg1, arg2, arg3, arg4, arg5, arg6), 1, 1, 0)
end
...
function Nx.prtCtrl (msg, arg1, arg2, arg3, arg4, arg5, arg6)

	if IsControlKeyDown() then
		Nx.prt (msg, arg1, arg2, arg3, arg4, arg5, arg6)
	end
--
end
...
function Nx.prtStrHex (msg, str)
	...
	for n = 1, Nx.compat:tsize (str), 4 do

		local s = ""

		for n2 = n, min (Nx.compat:tsize (str), n + 3) do
...
function Nx.Util_CapStr (str)
	return strupper (Nx.compat:strsub (str, 1, 1)) .. strlower (Nx.compat:strsub (str, 2))
end
...
function Nx.Util_c2rgb (colors)
	...
	local r = tonumber (Nx.compat:strsub (colors, 1, 2), 16) / 255
	local g = tonumber (Nx.compat:strsub (colors, 3, 4), 16) / 255
	local b = tonumber (Nx.compat:strsub (colors, 5, 6), 16) / 255
	return r, g, b
end
...
function Nx.Util_c2rgba (colors)
	...
	local g = tonumber (Nx.compat:strsub (colors, 3, 4), 16) / 255
	local b = tonumber (Nx.compat:strsub (colors, 5, 6), 16) / 255
	local a = tonumber (Nx.compat:strsub (colors, 7, 8), 16) / 255
	return r, g, b, a
end
...
function Nx.Util_num2rgba (colors)
	...
	local g = band (rshift (colors, 16), tonumber("0xff", 16)) / 255
	local b = band (rshift (colors, 8), tonumber("0xff", 16)) / 255
	local a = band (colors, tonumber("0xff", 16)) / 255
	return r, g, b, a
end
...
function Nx.Util_num2a (colors)
	...
	return bit.band (colors, tonumber("0xff", 16)) / 255
end
...
function Nx.Util_num2colstr (colors)
	...
	return format ("|c%02x%02x%02x%02x", band (colors, tonumber("0xff", 16)),
			rshift (colors, 24), band (rshift (colors, 16), tonumber("0xff", 16)), band (rshift (colors, 8), tonumber("0xff", 16)))
end
...
function Nx.Util_GetMoneyStr (money)
	...
	return pre .. Nx.compat:strtrim (str)
end
...
function Nx.Util_GetTimeElapsedMinSecStr (seconds)
	return format ("%d:%02d", mod(seconds / 60 , 60), mod(seconds , 60))
end
...
--		Nx.prt ("Item %s", str)
		...
		local link, s = Nx.compat:strsplit ("%^", str)

		if not s or Nx.compat:tsize (s) < 1 or IsAltKeyDown() then
			str = Nx.compat:strsub (link, 2)
			Nx.Item:ShowTooltip (str)
			return
		end
	...
	elseif strbyte (str) == 64 then	-- @ (quest)
		...
		str = "quest:" .. Nx.compat:strsub (str, 2)
		Nx.Item:ShowTooltip (str)
		return

	elseif strbyte (str) == 35 then	-- # (enchant)

		str = Nx.compat:strsub (str, 2)
--		Nx.prt (str)
	...
	if s1 then
		...
		for _, line in ipairs (t) do
			...
			local s1, s2 = Nx.compat:strsplit ("\t", line)
			if s2 then
...
--[[
		local s = Nx.compat:strsub (str, 1, s1 - 1)
--		Nx.prt ("Tool %s", s)
		GameTooltip:SetText (s, 1, 1, 1, 1, 1)

		s = Nx.compat:strsub (str, s2 + 1)
...
function Nx.Skin:Init()
	...
	Nx.Skins = {
		["Blackout"] = {
			...
			["BdCol"] = tonumber("0xff", 16),
			["BgCol"] = tonumber("0xff", 16),
		},
		["BlackoutBlues"] = {
			["Folder"] = "",
			["WinBrH"] = "WinBrH",
		...
		["DialogBlue"] = {
			...
			["BdCol"] = tonumber("0xccccffff", 16),
			["BgCol"] = tonumber("0x1f1f1fe0", 16),		-- { .125, .125, .125, .88 },
		},
		["DialogGold"] = {
			["Folder"] = "",
			["WinBrH"] = "WinBrH",
			...
			["BdCol"] = tonumber("0xffffffff", 16),
			["BgCol"] = tonumber("0x262600e0", 16),		-- { .15, .15, 0, .88 },
		},
		["SimpleBlue"] = {
			["Folder"] = "",
			["WinBrH"] = "WinBrH",
...
function Nx.Skin:Set (skinName, init)
	...
	if not init then
		...
		self.GOpts["SkinWinFixedBgColor"] = tonumber("0x80808080", 16)
...
function Nx.Window:Detach (childFrm)
	...
	Nx.prt ("Detach %s", Nx.compat:tsize (self.ChildFrms))

	for i, ch in ipairs (self.ChildFrms) do
		if ch.Frm == childFrm then
			tremove (self.ChildFrms, i)
			Nx.prt ("Detach found %s", Nx.compat:tsize (self.ChildFrms))
			break
		end
	end
end
...
function Nx.Window:Adjust (skipChildren)
	...
	if not skipChildren then
		...
		for n = 1, Nx.compat:tsize (self.ChildFrms) do
...
function Nx.Window:ResetLayout()
	...
	if data["_X"] then
		...
		for k, v in pairs (data) do
			...
			if k ~= "_X" then
				if Nx.compat:strsub (k, -1) == "X" then
					local mode = Nx.compat:strsub (k, 1, Nx.compat:tsize (k) - 1)
...
function Nx.Window:RegisterHide()
	...
	self.Frm:SetScript ("OnHide", function()
		this.NxWin:Notify ("Hide")
	end)
end
...
function Nx.Window:Notify (name, arg1, arg2, arg3, arg4, arg5, arg6)

	if self.UserFunc then
		self.UserFunc (self.User, name, arg1, arg2, arg3, arg4, arg5, arg6)
	end
end
...
function Nx.Window:OnEvent ()

	--V4 this
	local win = this.NxWin

	-- Nx.prt ("Win Event %s %s", win.Name, event or "nil", arg1 or "nil")
	...
	if win.Events and win.Events[event] then
		win.Events[event] (win.User, event, arg1, arg2, arg3, arg4, arg5, arg6)
	end
end

function Nx.Window:OnMouseDown ()
...
--	Nx.prtFrame ("Win", this)
	...
	if win.Secure and Nx.compat:InCombatLockdown() then
		return
	end
...
function Nx.Window:OnUpdate ()

	-- local this = self			--V4
	local win = this.NxWin
	local elapsed = arg1
	local secureOk = not (win.Secure and Nx.compat:InCombatLockdown())
	...
	if fade ~= win.BackgndFade then
		...
		for n = 1, Nx.compat:tsize (win.ChildFrms) do
...
--			end
		...
		elseif but.Type.States then
			...
			but.State = mod(but.State , but.Type.States) + 1
...
function Nx.Menu:AddSubMenu (menu, text)
	...
	local item = {}
	self.Items[Nx.compat:tsize (self.Items) + 1] = item
...
function Nx.Menu:Update()
	...
	for n = 1, Nx.compat:tsize (self.Items) do
...
function Nx.Menu:Item_OnUpdate ()
...
function Nx.List:FreeFrames (list)
	...
	list.UsedFrms = Nx.compat:wipe (list.UsedFrms or {})
end
...
function Nx.List:Create (saveName, xpos, ypos, width, height, parentFrm, showAll, noHeader)
	...
	if saveName then
		...
		if save["ColW"] then
			inst.SaveColumnWidths = { Nx.compat:strsplit ("%^", save["ColW"]) }
		end
	end
...
--	prt ("ListSetSize "..self.Vis.." "..#self.Strs)
	...
	for n = strNum, Nx.compat:tsize (self.Strs) do
		self.Strs[n]:Hide()
	end
...
--				Nx.prt ("List resize create but %s", butNum)
	...
	if self.Buts then
		for n = butNum, Nx.compat:tsize (self.Buts) do
			if self.Buts[n] then
...
--	Nx.prt ("List lineH %s", lineH)
	...
	if self.Offsets or Nx.compat:tsize (self.Strs) < self.Vis then
		self:CreateStrings()
	end
	...
	if self.ButData then
		...
		if not self.Buts or Nx.compat:tsize (self.Buts) < self.Vis then
			self:CreateButtons()
		end
...
--		Nx.prt ("List Update Vis %s", self.Vis)
		...
		for n = 1, self.Vis do
			...
			if butType then
				...
				if not but then
					Nx.prt ("!BUT %s", Nx.compat:tsize (self.Buts))
				end
				assert (but)
	...
	--[[ if self.FrmData then			-- Only used for Info and Watch Items
		...
		for n = 1, self.Vis do
			...
			if data then
				...
				local typ, v1, v2, v3 = Nx.compat:strsplit ("~", data)
...
--				Nx.prt ("%s", -(n - 1) * lineH - adjY - offY)
				...
				elseif typ == "WatchItem" then
					...
					if dur then
						QuestWatchFrameItem_UpdateCooldown (f)
					end
					...
					if doBind then
						...
						if Nx.compat:tsize (opts["QWKeyUseItem"]) > 0 and not Nx.compat:InCombatLockdown() then

							local s = GetBindingAction (opts["QWKeyUseItem"])
							s = Nx.compat:strmatch (s, "CLICK (.+):")
...
function Nx.List:Empty()
	...
	self.Num = 0
	self.Data = Nx.compat:wipe (self.Data or {})

	for k, column in pairs (self.Columns) do
		column.Data = column.Data and Nx.compat:wipe (column.Data)
	end

	if self.ButData then
		Nx.compat:wipe (self.ButData)
	end

	if self.Offsets then
		Nx.compat:wipe (self.Offsets)
	end

	if self.FrmData then
		Nx.compat:wipe (self.FrmData)
	end

--	if self.FrmDataFrm then
--		Nx.compat:wipe (self.FrmDataFrm)
--	end
...
function Nx.List:Sort()
	...
	for n = 1, self.Num do
		...
		t[n] = gsub (cData[n] or "", "|cff......", "") .. " " .. Nx.compat:strsub (name, 1, 1) .. "~" .. n
	end
	...
	for n = 1, Nx.compat:tsize (t) do
		local _, i = Nx.compat:strsplit ("~", t[n])
		t[n] = tonumber (i)
	end
...
--			Nx.prt ("%s %s %s %s", r, g, b, a)
			f.NXTbl[f.NXVName] = floor (r * 255) * tonumber("0x1000000", 16) + floor (g * 255) * tonumber("0x10000", 16) + floor (b * 255) * tonumber("0x100", 16) + floor (a * 255)
...
function Nx.Graph:SetLine (time, value, colorStr, infoStr)
	...
	self.Values[pos + tonumber("0x1000000", 16)] = colorStr
	self.Values[pos + tonumber("0x2000000", 16)] = infoStr
...
function Nx.Graph:UpdateLine (pos)
	...
	if x >= 0 and x < self.Width - 1 then
		...
		if h >= .1 then
			...
			local colorStr = self.Values[pos + tonumber("0x1000000", 16)]
...
function Nx.Graph:OnEnter (motion)
	...
	if not GameTooltip:IsOwned (this) and this.NxGraphPos then
		...
		local v = self.Values
		local str = format ("%.2f: %s", v[-this.NxGraphPos], v[this.NxGraphPos + tonumber("0x2000000", 16)])
--		Nx.prt (str)
