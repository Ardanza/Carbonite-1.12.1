...
function Nx.Quest:Init()
	...
	Nx.compat:hooksecurefunc ("WatchFrame_Update", self.WatchFrame_Update)
...
--		[11877] = "4^66",
	...
	for id, s in pairs (qdata) do
		...
		local objI, zone, x, y = Nx.compat:strsplit ("%^", s)
		objI = tonumber (objI)
...
--					Nx.prt ("%s %s %s", id, x, y)
					...
					quest[objI] = format ("%c%s%c %c%c%c%c", Nx.compat:tsize (oDesc) + 35, oDesc, zone + 35,
							floor (x / 221) + 35, mod(x , 221) + 35, floor (y / 221) + 35, mod(y , 221) + 35)
				else
					local oDesc, oZone, oLoc = self:UnpackObjective (obj)
					quest[objI] = format ("%c%s%c%s", Nx.compat:tsize (oDesc) + 35, oDesc, zone + 35, Nx.compat:strsub (obj, oLoc))
				end
			end
		end
	end
...
--]]
	...
	for id, q in pairs (Nx.Quests) do
		...
		if q[2] then
			local name, zone, x, y = self:GetSEPos (q[2])
			if zone == 41 then 
...
--]]
	...
	for id, q in pairs (Nx.Quests) do
		...
		local name, side, lvl, minlvl, nextQ = self:Unpack (q[1])
...
--		end
		...
		if not q.CNum and nextQ > 0 then
...
--				end
				...
				name, side, lvl, minlvl, nextQ = self:Unpack (qc[1])
				...
				if side ~= enFact then
					...
					elseif q.CNum == 1 then
						...
						while qc do
							...
							local pname, side, _, _, nextQ = self:Unpack (qc[1])

--							if strfind (name, "Load Lightening") then
--								Nx.prt ("%s %d %d (%d %d)", pname, id, side, next, qc.CNum)
--							end

							tinsert (grp, format ("%s%2d^%d", name, qc.CNum, onId))
							qc = self.IdToQuest[nextQ]
							onId = nextQ
						end
					end
...
--		table.sort (grp)
		...
		for _, v in ipairs (grp) do
			local name, id = Nx.compat:strsplit ("%^", v)
...
--		Nx.prt ("auto")
	...
	for k, name in ipairs (ttHooks) do
		if not Nx.V30 or name ~= "SetCraftItem" and name ~= "SetCraftSpell" then
			-- Nx.compat:hooksecurefunc (GameTooltip, name, Nx.Quest.TooltipHook)
		end
	end
...
-- 		q[n] = format ("%s# ####", Nx.compat:strsub (q[n], 1, 2))	-- Zero it to get a red button
...
function Nx.Quest:Menu_OnTrack()
	...
	local v = cur.QId * tonumber("0x10000", 16) + self.IconMenuObjI * tonumber("0x100", 16) + cur.QI
...
--	Nx.prt ("ExpandQuests")
	...
	repeat
		...
		for qn = 1, cnt do
			...
			local title, level, tag, isHeader, isCollapsed = GetQuestLogTitle (qn)
...
--			Nx.prt ("RecordQuests RemoveQ %s - %s", cur.Title, cur.QI)
	...
	local index = Nx.compat:tsize (curq) + 1

	for qn = 1, qcnt do

		local title, level, tag, isHeader, isCollapsed, isComplete, isDaily = GetQuestLogTitle (qn)
...
--			end
			...
			local qId, qLevel = self:GetLogIdLevel (qn, qObj)
			assert (qId)
			local quest = self.IdToQuest[qId]
...
-- 					local mask = (cur.PartyComplete or Nx.compat:tsize (qT) == 0) and 1 or 0
...
-- 					tinsert (curq, cur)
-- 					cur.Index = Nx.compat:tsize (curq)

-- 					cur.LBCnt = Nx.compat:tsize (qT)

-- 					local mask = (qT.Complete or Nx.compat:tsize (qT) == 0) and 1 or 0
...
function Nx.Quest:ScanBlizzQuestDataTimer()
	...
	QuestWatchFrame:UnregisterEvent ("WORLD_MAP_UPDATE")		-- Map::ScanContinents can enable this again
...
--			Nx.prt ("ScanQ next %s %s", cont, mapId)
			...
			if cont == Nx.Map.ContCnt then			-- Done?
				...
				QuestWatchFrame:RegisterEvent ("WORLD_MAP_UPDATE")	-- Back on when done
	...
	for id, q in pairs (Nx.Quests) do
		...
		if q.CNum == 1 then		-- Only look at chain starters
			...
			while qc do
				...
				if onId == qId then		-- Found me in chain? Mark before me complete

					local qc = q
					while onId ~= qId do

						local qStatus = Nx:GetQuest (onId)
						if qStatus ~= "C" then

							cnt = cnt + 1

--							Nx.prt ("%s %s", id, qId)
							Nx:SetQuest (onId, "C", time())
						end

						onId = self:UnpackNext (qc[1])
						qc = self.IdToQuest[onId]
					end
...
function Nx.Quest:GetLogIdLevel (index, qObj)

	if index > 0 then
		local logname, level = GetQuestLogTitle (index)
		local obj = GetQuestLogLeaderBoard(1, index) or qObj
		local qId
		qId = self.QcacheToId[logname .. (obj or "")]
		if qId then
			return tonumber(qId), tonumber(level)
		end
		for id, q in pairs(self.IdToQuest) do
			local name = self:Unpack (q[1])
			if name == logname then
				qId = (not qId) and id or qId
				if (not obj) and (not q[4]) then
					qId = id
				elseif obj then
					for i=4,15 do
						if q[i] and Nx.compat:strmatch(obj, self:UnpackObjective (q[i]) or "") then
							self.QcacheToId[logname .. (obj or "")] = id
							-- Nx. prt("Quest found %s - %s", logname, id)
		...
		-- if not qId then 
		-- 	Nx. prt("Quest %s lvl \"%s\" is |cffff0000not in the database|r", level, logname)
		-- end
		self.QcacheToId[logname .. (obj or "")] = qId
		return tonumber(qId or 0), tonumber(level or UnitLevel("player"))
	end
end
...
--	Nx.prt ("Orig '%s'", title)
	...
	if e then
		title = Nx.compat:strsub (title, e + 1)
...
function Nx.Quest:SortQuests()
	...
	-- 	for n = 1, Nx.compat:tsize (curq) - 1 do
	...
	if self.List.QOpts.NXShowHeaders then
		...
		for n = 1, Nx.compat:tsize (curq) do
...
--		Nx.prtVar ("HDR", hdrs)
		...
		for _, name in ipairs (hdrs) do
			...
			for n = 1, Nx.compat:tsize (curq2) do
...
--]]
	...
	local cnt = GetNumQuestLogEntries()
	-- UIErrorsFrame:AddMessage(GetNumQuestLogEntries())
--	Nx.prt ("FindNewQuest %d", cnt)

	for qn = 1, cnt do

		local title, level, tag, isHeader, isCollapsed, isComplete = GetQuestLogTitle (qn)
...
function Nx.Quest:RecordQuestAcceptOrFinish()
	...
	-- if guid then
	-- 	local typ = tonumber (Nx.compat:strsub (guid, 3, 5), 16)
	-- 	if typ == 0 then	-- Player
	-- 		giver = "p"

	-- 	elseif bit.band (typ, tonumber("0xf", 16)) == 1 then
	-- 		local id = tonumber (Nx.compat:strsub (guid, 6, 12), 16)
	-- 		giver = format ("%s#o%x", giver, id)

	-- 	elseif bit.band (typ, tonumber("0xf", 16)) == 3 then		-- NPC?
	-- 		local id = tonumber (Nx.compat:strsub (guid, 7, 10), 16)
...
--	Nx.prt ("OnChat_msg_combat_faction_change %s", arg1)
	...
	form = gsub (form, "%%s", "(.+)")
	form = gsub (form, "%%d", "(%%d+)")
	local facName, rep = Nx.compat:strmatch (arg1, form)
	rep = tonumber (rep)
...
--			Nx.prt ("Fac %s %s", facName, rep)
			...
			local qdata = { Nx.compat:strsplit ("~", quests[self.CaptureQEndId]) }
			local ender, reps = Nx.compat:strsplit ("@", qdata[2])

			local repdata = reps and { Nx.compat:strsplit ("%^", reps) } or {}
...
function Nx.Quest:Capture (curi, objNum)
	...
	for id, str in pairs (quests) do
		len = len + 4 + Nx.compat:tsize (str) + 1
	end
...
--	ids["I"] = format ("%d^%s^%s", cur.RealLevel, cur.Title, cur.Header)
	...
	local qdata = { Nx.compat:strsplit ("~", q) }

	if not objNum then	-- Starter

--		local flags = bit.bor (tonumber (Nx.compat:strsub (qdata[1], 1, 1), 16) or 0, facMask)
...
--		local nxzone = Nx.MapIdToNxzone[map.RMapId]
		...
		if nxzone then
			...
			if Nx.compat:tsize (obj) >= 3 then
				local z = tonumber (Nx.compat:strsub (obj, 1, 3), 16)
				if nxzone ~= z then
					return
				end
			else
			...
			local cnt = (Nx.compat:tsize (obj) - 3) / 6
			if cnt >= 15 then
				return
			end
			...
			for n = 1, cur.LBCnt do
				...
				if desc then
					if not done then
						...
						-- Nx.prt ("TellParty %s/ %s",cur[n] or "nil", desc or "nil")
						local oldCnt = tonumber (Nx.compat:strmatch (cur[n] or "", ": (%d+)/") or "")
						local newCnt = tonumber (Nx.compat:strmatch (desc, ": (%d+)/") or "")
...
--	Nx.prt ("Goto %s", qId)
	...
	self.Watch:Add (Nx.compat:tsize (curq))
...
function Nx.Quest:Abandon (qIndex, qId)
	...
	if qIndex > 0 then
		...
		local title, level, tag, isHeader = GetQuestLogTitle (qIndex)
...
--	Nx.prt ("OnMsgQuest (%s) %s", plName, msg)
	...
	local id = Nx.compat:strsub (msg, 2, 2)
...
--		if nil then
	...
	elseif id == "B" then	-- Busy
		...
		if plName == self.RcvPlyr then
			...
			local mode = Nx.compat:strsub (msg, 3, 3)
	...
	elseif id == "D" then	-- Incoming quest data
		...
		if plName == self.RcvPlyr then
			...
			if Nx.compat:tsize (msg) >= 4 then

				local data = Nx.compat:strsub (msg, 3)
				local mode = Nx.compat:strsub (msg, 3, 3)

				if mode == "0" then

					self.RcvCnt = 0
					self.RcvTotal = tonumber (Nx.compat:strsub (data, 3)) or 0
...
--			Nx.prt ("%s, %s, %s", arg1, arg2 or "nil", arg3 or "nil")
			...
			if data then
				local shelf, item, x, y = Nx.compat:strsplit ("~", data)
				x = tonumber (x) * .01
				y = tonumber (y) * .01
...
--				Nx.prt ("TTM #%s", GameTooltip:NumLines())
			...
			if Nx.compat:strsub (s, 1, 3) == " - " then	-- Blizz added quest info?
	...
	if tipStr and Nx.compat:tsize (tipStr) > 5 and Nx.compat:tsize (tipStr) < 50 and not self.TTIgnore[tipStr] then
				...
				if s1 then
					...
					for n = 1, cur.LBCnt do
						if strfind (cur[n], tipStr) then
							...
							if s1 then
								local oName = Nx.compat:strsub (cur[n], 1, s1 - 1)
								tip:AddLine (format ("    |cffb0b0b0%s%s%s", oName, color, Nx.compat:strsub (cur[n], s1)))
							else
...
function Nx.Quest.List:UpdateMenu()
	...
	if self.TabSelected == 1 then
		...
		if i then
			...
			local qi = bit.band (i, tonumber("0xff", 16))
			if qi > 0 then
...
function Nx.Quest.List:Menu_OnGoto (item)
	...
	if i then
		...
		local qIndex = bit.band (i, tonumber("0xff", 16))
...
--[[
	elseif mode == 0 then
		local str = Nx.compat:strsub (cur.ObjText, 1, 180)
		str = format (" %s", str)
		str = gsub (str, "[\n\r\t]", "")
...
function Nx.Quest.List:Menu_OnShare (item)
	...
	if i then
		...
		local qi = bit.band (i, tonumber("0xff", 16))
		if qi > 0 then
			if Nx.compat:GetNumSubgroupMembers() > 0 then
				QuestLogPushQuest()
			else
...
function Nx.Quest.List:OnListEvent (eventName, sel, val2, click)
	...
	local qIndex = bit.band (itemData, tonumber("0xff", 16))
...
--	Nx.prt (format ("Data #%d, Id%d", qIndex, qId))
	...
	if eventName == "select" or eventName == "mid" or eventName == "back" then
		...
		if qId > 0 then
			...
			local qObj = bit.band (bit.rshift (itemData, 8), tonumber("0xff", 16))
			...
			if shift and qObj == 0 or qObj > 0 then
				...
				if qObj == 0 then
					if bit.band (tbits, 1) > 0 then
						...
						Quest.Tracking[qId] = tonumber("0xffffffff", 16)		-- Track all
					end
				else
					Quest.Tracking[qId] = bit.bxor (tbits, bit.lshift (1, qObj))
				end
...
function Nx.Quest.List:MakeDescLink (cur, id, debug)
	...
	if debug then
		local fac = Nx.compat:strsub (UnitFactionGroup ("player"), 1, 1)
...
--	Nx.prt ("QuestListUpdate")
	...
	self.Win:SetTitle (format ("Quests: |cffffffff%d/%d|r", i, MAX_QUESTS))
	...
	if self.TabSelected == 1 then
		...
		for n = 1, curq and Nx.compat:tsize (curq) or 0 do
			...
			if not self.ShowParty or onQ > 0 then
				...
				if show and not Quest.HeaderHide[cur.Header] then
					...
					list:ItemAdd (qId * tonumber("0x10000", 16) + qn)
					...
					if self.QOpts.NXShowObj then
						...
						for ln = 1, 15 do
							...
							list:ItemAdd (qId * tonumber("0x10000", 16) + ln * tonumber("0x100", 16) + qn)
...
--				Nx.prt ("%s [%s] %s", qname, qId, quest.CNum or "")
				...
				if dailyStr then
					...
					local typ = Nx.compat:strsplit ("%^", dailyStr)
		...
		for _, qEntry in ipairs (sortT) do
			...
			list:ItemAdd (qEntry.QId * tonumber("0x10000", 16))
...
--					end
				...
				if dailyStr then
					local typ, money, rep, req = Nx.compat:strsplit ("%^", dailyStr)
					tag = format ("|cffd060d0(%s %.2fg", Quest.DailyTypes[typ], money / 100)
					for n = 0, 1 do	-- Only support 2 reps
						local i = n * 4 + 1
						local repChar = Nx.compat:strsub (rep or "", i, i)
						if repChar == "" then
							break
						end
						tag = format ("%s, %s %s", tag, Nx.compat:strsub (rep, i + 1, i + 3), Quest.Reputations[repChar])
					end
				...
				if show then
					...
					list:ItemAdd (qId * tonumber("0x10000", 16))
					...
					if eName then
						list:ItemAdd (qId * tonumber("0x10000", 16) + 16 * tonumber("0x100", 16))
						list:ItemSet (2, "     |cff6060ffEnd: " .. eName)
						list:ItemSet (4, eMapName)

						list:ItemSetButton ("QuestWatch", false)
						if bit.band (trackMode, tonumber("0x10000", 16)) > 0 then
							list:ItemSetButton ("QuestWatch", true)
						end
						list:ItemSetButtonTip (questTip)
					end
					...
					for n = 1, 15 do
						...
						list:ItemAdd (qId * tonumber("0x10000", 16) + n * tonumber("0x100", 16))
...
--			qsindex = qsindex + 1
	...
	if self.TabSelected == 4 then
		...
		for n = 1, Nx.compat:tsize (Quest.FriendQuests) do

			local data = Quest.FriendQuests[n]
			local mode = Nx.compat:strsub (data, 1, 1)

			list:ItemAdd (0)

			if mode == " " then		-- Simple text

				list:ItemSet (2, Nx.compat:strsub (data, 3))

			elseif mode == "H" then

				list:ItemSet (2, format ("|cff8f8fff---- %s ----", Nx.compat:strsub (data, 3)))

			elseif mode == "T" then

				local _, qId, watched, done, lvl, name = Nx.compat:strsplit ("%^", data)
...
--						watched = watched == "0" and "" or "*"
			...
			elseif mode == "O" then
				...
				local _, qId, name = Nx.compat:strsplit ("%^", data)
...
--		Nx.prt ("%s %s", i, data)
		...
		if data > 0 then
			Nx.Quest:SelectBlizz (bit.band (data, tonumber("0xff", 16)))
			NxQuestD:Show()
...
--		Nx.prt ("QTar %s", tid)
		...
		if cur then
			...
			Quest:TrackOnMap (cur.QId, mod(tid , 100), cur.QI > 0 or cur.Party, true, true)
...
--			Nx.prt ("UpIcons target %s %s", typ or "nil", tid or "nil")
	...
	if mod(Nx.Tick , 10) == 0 then

--		tracking = {}		-- garbage creator
		Nx.compat:wipe (tracking)
		...
		if showOnMap then
			for k, cur in ipairs (Quest.CurQ) do
				if cur.Q and (Nx:GetQuest (cur.QId) == "W" or cur.PartyDesc) then
					tracking[cur.QId] = (tracking[cur.QId] or 0) + tonumber("0x10000", 16)		-- cur.TrackMask + i
				end
			end
		end
	...
	for trackId, trackMode in pairs (tracking) do
		...
		if showEnd or bit.band (mask, tonumber("0x10000", 16)) > 0 then
...
--					Nx.prt ("%s zone %d %s", objName, mapId, loc)
					...
					if bit.band (mask, bit.lshift (1, n)) > 0 then
						...
						if colorPerQ then
							colI = mod(((cur and cur.Index or 1) - 1) , colMax) + 1
						end
						...
						local points = {Nx.compat:strsplit("~", obj)}
						table.remove(points, 1)
						table.remove(points, 1)
						local typ = table.remove(points, 1)

						-- local QMap = NxMap1.NxMap
						-- if not Nx.compat:InCombatLockdown() then	
...
--							Nx.prt ("%s, pt %s", objName, Nx.compat:strsub (obj, loc + 1))

							local sz = navscale

							if Nx.compat:tsize (points) > 1 then
...
--							Nx.prt ("%s, spans %s", objName, Nx.compat:strsub (obj, loc))
							...
							if not cur or drawArea or hover
								...
								-- 	points[Nx.compat:tsize(points) + 1] = format("%.2f^%.2f^%.2f^%.2f",Nx.Map.CPx,Nx.Map.CPy,5.01,3.34) 
								-- 	GameTooltip:Hide()
								-- end

								local ssub = Nx.compat.strsub
								...
								for _, p in ipairs(points) do
									...
									w,h = w*(winfo.ScaleAdjX or 1), h*(winfo.ScaleAdjY or 1)
...
function Nx.Quest.Watch:OnTimer (item)
	...
	if i <= Nx.compat:tsize (curq) then
		self.CalcDistI = i
		return .02
	end
...
--	Nx.prt ("QWatchUpdate")
	...
	local watched = Nx.compat:wipe (self.Watched)
...
--			Nx.prt ("qid %s %s dist %s", qId, qStatus, cur.Distance)
		...
		if self.ButATarget:GetPressed() then
			...
			if disti then
				local cur = curq[bit.band (disti, tonumber("0xff", 16))]
...
--			Nx.prtVar ("Watched", watched)
			...
			for _, distn in ipairs (watched) do
				...
				local n = bit.band (distn, tonumber("0xff", 16))
...
--					local link, item, charges = GetQuestLogSpecialItemInfo (questIndex)
					...
					list:ItemAdd (qId * tonumber("0x10000", 16) + qi)
...
--											Nx.prt ("%s %s", i, total)
								...
								if gopts["QWOCntFirst"] then
									local s1, s2 = Nx.compat:strmatch (desc, "(.+): (.+)")
									if s2 then
								...
								list:ItemAdd (qId * tonumber("0x10000", 16) + ln * tonumber("0x100", 16) + qi)
...
--								Nx.prt ("watch %s %s %s", qId, zone or "nil", butType or "nil")
								...
								if not fixedSize then
									...
									while Nx.compat:tsize (str) > maxC do
										...
										local s = Nx.compat:strsub (str, 1, maxC)
										list:ItemSet (2, s)

										str = color .. Nx.compat:strsub (str, maxC + 1)
										list:ItemAdd (qId * tonumber("0x10000", 16) + ln * tonumber("0x100", 16) + qi)
										list:ItemSetOffset (16, lnOffset)
...
--	Nx.prt ("QuestListUpdate "..eventName)
	...
	if eventName == "button" then
		...
		if data then
			...
			local qIndex = bit.band (data, tonumber("0xff", 16))
...
--		Nx.prt ("Q Set %s %s", i, cur and cur.Name or "nil")
		...
		-- 0 is quest name line
		local qObj = bit.band (bit.rshift (data, 8), tonumber("0xff", 16))
		...
		if i then
			...
			if qId > 0 and (not qIdMatch or qIdMatch == qId) then
				...
				if cur then
					...
					local qObj = bit.band (bit.rshift (i, 8), tonumber("0xff", 16))
...
--			Nx.prt ("%s", item.Text)
			...
			if not IsShiftKeyDown() then
				local quest = self.IdToQuest[item.UData]
				if quest then
					local qname, _, lvl = self:Unpack (quest[1])
					Nx.prt ("|cffffff00Quest|r %d lvl \"%s\"", lvl, qname)
				end
				local str = "|cffffff00Objectives|r "
				for i=4,10 do
					if quest[i] then
						local oName = self:UnpackObjective (quest[i])
						str = str .. oName .. " "
					end
				end
				if str ~= "|cffffff00Objectives|r " then
					Nx.prt (str)
				end
				if quest[3] then
					local ender, zone, x, y = self:GetSEPos (quest[3])
					local zone = Nx.compat:strsplit ("!", Nx.Zones[zone])
					Nx.prt ("|cffffff00End at|r %s in %s (%s, %s)", ender, zone, x, y)
				end
			else
...
function Nx.Quest:UpdateGiverIconMenu()
	...
	local curI = 1
	local qids = {Nx.compat:strsplit("~", qdata)}
			...
			if typ == "Q" then
				...
				if tqid == qId then		-- Same quest as us?
					...
					if tbits == 0 or (tid == qId * 100 + qObj) then
						if self.GOpts["QSync"] then
							Nx.compat:RemoveQuestWatch(BlizIndex)
						end
						self.Map:ClearTargets()
						-- if not Nx.compat:InCombatLockdown() then						
						-- 	local QMap = NxMap1.NxMap						
...
function Nx.Quest:Unpack (info)
	local name, side, level, minlvl, nextId = Nx.compat:strsplit("~", info)
...
function Nx.Quest:UnpackName (info)
	return Nx.compat:strsplit("~", info)
end
...
function Nx.Quest:UnpackNext (info)
	local tbl = {Nx.compat:strsplit("~", info)}
	return tonumber(tbl[5])
end
...
function Nx.Quest:UnpackObjective (obj)
	...
	local desc, zone = Nx.compat:strsplit("~", obj)
	zone = tonumber(zone) or 0
	zone = zone > Nx.compat:tsize (Nx.Zones) and 0 or zone
	return desc, zone
end
...
function Nx.Quest:GetObjectiveType (obj)
	...
	local _, _, typ = Nx.compat:strsplit("~", obj)
...
function Nx.Quest:GetPosLoc (str)
	...
	local points = {Nx.compat:strsplit("~", str)}
	table.remove(points, 1)
	table.remove(points, 1)
	local typ = table.remove(points, 1)

	if typ == "p" then  -- Point
		cnt = Nx.compat:tsize (points)
		for _, p in ipairs(points) do
...
function Nx.Quest:GetClosestObjectiveRect (str, mapId, px, py)
	...
	if strbyte (str, loc) <= 33 then  -- Point
		...
		for locN = loc + 1, loc + loopCnt * 4, 4 do
			...
			local loc1 = Nx.compat:strsub (str, locN, locN + 3)
			assert (loc1 ~= "")
...
function Nx.Quest:UnpackLoc (locStr)
	local x, y = Nx.compat:strsplit("%^", locStr)
...
function Nx.Quest:UnpackLocRect (locStr)
	local x, y, w, h = Nx.compat:strsplit("%^", locStr)
	...
	if quest then
		...
		if #hashPat > 0 then
			...
			for n = 1, #hashPat, 4 do
				...
				local off = tonumber (Nx.compat:strsub (hashPat, n + 1, n + 2), 16) + 1
				local match = Nx.compat:strsub (hashPat, n + 3, n + 3)

--				Nx.prt ("QFind #%d %d %d %s", n, mode, off, match)

				if mode == 48 then

					found = Nx.compat:strsub (obj, off, off) == match
				...
				elseif mode == 50 then
					...
					found = Nx.compat:strsub (desc, off, off) == match
...
function Nx.Quest:DecodeComRcv (info, msg)
	...
	if Nx.compat:tsize (msg) < 7 then	-- Too short?
	...
	local qId = tonumber (Nx.compat:strsub (msg, 1, 4), 16) or 0
	local quest = self.IdToQuest[qId]
	...
	if Nx.compat:tsize (msg) >= 7 + lbcnt * 2 then
...
--	Nx.prt ("OnParty_members_changed")
	...
	for name in pairs (pq) do
		...
		for n = 1, Nx.compat:GetNumSubgroupMembers() do
...
--			Nx.prt ("Old %s", name)
	...
	if Nx.compat:IsInRaid() then	-- In raid?
		return
	end

	if Nx.compat:GetNumSubgroupMembers() == 0 then	-- Left party?
		return
	end
...
function Nx.Quest:OnPartyMsg (plName, msg)
	...
	if pl then
		...
		for n = 1, 99 do
			...
			if Nx.compat:tsize (msg) < off + 5 then	-- No more?
				break
			end

			local qId = tonumber (Nx.compat:strsub (msg, off, off + 3), 16) or 0
			local flgs, oCnt =  Nx.compat:strbyte (msg, off + 4, off + 5)
			flgs = flgs - 35
			oCnt = oCnt - 35

			if Nx.compat:tsize (msg) < off + 5 + oCnt * 4 then	-- Too short?
				break
			end
...
--				Nx.prt ("%s: %s %x %s", plName, qId, flgs, oCnt)
				...
				for i = 1, oCnt do
					...
					local o = off + 6 + (i - 1) * 4
					local cnt = tonumber (Nx.compat:strsub (msg, o, o + 1), 16) or 0
					local total = tonumber (Nx.compat:strsub (msg, o + 2, o + 3), 16) or 0
...
function Nx.Quest:PartyStartSend()
	...
	if Nx.compat:IsInRaid() or Nx.compat:GetNumSubgroupMembers() == 0 then
		return
	end
...
function Nx.Quest:PartyBuildSendData()
	...
	for n, cur in ipairs (self.CurQ) do
		...
		if not cur.Goto and Nx:GetQuest (qId) == "W" then
			...
			if Nx.compat:tsize (sendStr) > 80 then
	...
	if Nx.compat:tsize (sendStr) > 0 or Nx.compat:tsize (data) == 0 then
		tinsert (data, sendStr)
	end
